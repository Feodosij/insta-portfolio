...
- [x] Форма з полями: ім'я, текст послуг, Instagram URL, Telegram URL, завантаження hero-фото (той самий Cloudinary-флоу, що й для галереї, окрема `folder: "site"`, щоб не змішувати з фото галереї; вантажить прямо в Cloudinary без створення рядка в `photos` — hero-фото не частина галереї)
- [ ] Перевірка вручну: зміна імені в адмінці одразу відображається на публічній головній після рефрешу — **не виконано асистентом**, той самий принцип (потрібна реальна сесія). Автоматично перевірено: `/admin/content` без сесії → `307`; `PUT /api/admin/content` без сесії → `401`. **Просимо користувача**: змінити ім'я/текст на `/admin/content`, зберегти, оновити публічну `/` — переконатись що зміна видно

**Додаткова деталь не з чекбоксів плану:** `PUT /api/admin/content` трактує порожнє значення поля як "очистити" (робить `delete`, а не `upsert` з порожнім рядком) — інакше `content.hero_name ?? FALLBACK` на публічній сторінці перестав би падати назад на фолбек, бо порожній рядок `""` не `null`/`undefined` і `??` його не підмінить. `hero_name`/`services_text` — обов'язкові поля форми (`required` на інпуті), тому на практиці очищення стосується лише `instagram_url`/`telegram_url`/`hero_photo_url`
...
- [x] Заглушка "фото ще немає" на рівні таба без фото
- [x] Заглушка на рівні всього сайту, якщо взагалі 0 фото в БД — `page.tsx` рахує загальну кількість фото одним `count`-запитом (`.select("*", { count: "exact", head: true })`, без витягування самих рядків) і передає `hasAnyPhotos` у `Gallery`; якщо `false` — `Gallery` взагалі не рендерить таби (показувати перемикач табів для порожньої галереї безглуздо), лише `EmptyState`
- [x] Перевірка вручну: тимчасово видалити всі фото з тестової БД, переконатись що немає пустих/зламаних блоків — **не робив видалення в реальній БД користувача** (там 24 живих фото), замість цього перевірив логіку рендер-тестом (`renderToStaticMarkup(<Gallery hasAnyPhotos={false} .../>)`, тимчасовий файл, видалений одразу після прогону) — підтверджено, що при `hasAnyPhotos=false` рендериться повідомлення "Фото ще немає" і жодного таба; build/lint/test чисті, публічна `/` з реальними 24 фото й далі рендерить звичайну галерею (curl-перевірка — `count("Фото ще немає") == 0`)
...
- [x] `<Analytics />` в root layout (`@vercel/analytics/react` вже був у залежностях зі старту проєкту — Сесія 0)
- [ ] Підключити репозиторій до Vercel (Import Project), прописати env-змінні з `.env.example` у Vercel Dashboard — **вимагає акаунту Vercel користувача**, асистент не має до нього доступу і не може виконати OAuth/деплой самостійно
...
**Що зроблено асистентом заздалегідь (код готовий до деплою):**
- `<Analytics />` підключено в `src/app/layout.tsx`
- Попутно виправлено `<html lang="en">` → `lang="uk"` (весь контент сайту українською — це не було зафіксовано жодною сесією плану, але явна помилка з часів `create-next-app`-скаффолду)
- `vercel.json` **не створювався** — у плані позначений як "за потреби", а конкретних редіректів/заголовків, що вимагали б його, не виникло; стандартний автодетект Next.js на Vercel не потребує додаткового конфігу
- `npm run build`/`lint`/`test` чисті на момент запису (21/21 unit-тестів)

**Що лишається зробити користувачу (поза можливостями асистента):**
1. Зайти на vercel.com, підключити GitHub-репозиторій `Feodosij/insta-portfolio` (Import Project)
2. Прописати в Vercel Dashboard → Settings → Environment Variables всі змінні з `.env.example` (значення взяти з локального `.env`)
3. Задеплоїти, відкрити посилання з телефону через Instagram in-app browser — перевірити hero/галерею/infinite scroll/viewer/послуги/OG-прев'ю
4. Залогінитись в адмінку з телефону — перевірити повний цикл (логін, upload, drag-and-drop, редагування тексту)
5. Заразом виконати всі відкладені ручні перевірки з Сесій 9-14 (вони й так вимагали реальної сесії/мобільного дотику) — логічно зробити одним проходом після деплою
...
**Оновлення 2026-07-11 (той самий день):** користувач попросив відкласти всі ручні перевірки до після деплою і завершити решту сесій поспіль без пауз між ними. Далі — Сесії 13, 14 і підготовча частина 15 виконані одним заходом, без очікування підтверджень.

### Session 13+14 — 2026-07-11

**Зроблено (Сесія 13):**
- `PUT /api/admin/content` — приймає `{ items: [{ key, value }] }`; `key` звіряється з білим списком `SiteContentKey` (`hero_name`, `hero_photo_url`, `services_text`, `instagram_url`, `telegram_url`) — довільний ключ у БД так не потрапить. Значення з порожнім (після `trim()`) рядком трактуються як "видалити рядок" (окремий `delete().in("key", toDelete)`), решта — `upsert(..., { onConflict: "key" })` одним запитом. Причина розділення: якщо просто зберегти `""`, публічні фолбеки на кшталт `content.hero_name ?? HERO_NAME_FALLBACK` (Сесія 3, 8) перестали б спрацьовувати — порожній рядок для `??` вже "заданий", а не "відсутній"
- `src/components/admin/SiteContentForm.tsx` — 5 полів (ім'я, hero-фото, текст послуг, Instagram, Telegram), одна кнопка "Зберегти" на все одразу (не окремі кнопки на кожне поле — менше кліків для флориста). Завантаження hero-фото повторює Cloudinary-флоу `PhotoUploadForm` (Сесія 10), але з `folder: "site"` (не змішувати з `folder: "gallery"`) і **без** виклику `POST /api/admin/photos` — hero-фото не рядок у таблиці `photos`, лише значення `site_content.hero_photo_url`; результат `secure_url` одразу йде в локальний стан форми, реальний запис у БД — тільки після натискання "Зберегти"
- `src/app/admin/(protected)/content/page.tsx` — тягне поточний `site_content` напряму браузерним Supabase-клієнтом (публічна RLS на читання, той самий підхід, що в Сесіях 11-12 для `categories`/`photo_categories`), без окремого GET-роута
- `layout.tsx`: третій пункт нав-меню — "Текст" → `/admin/content`

**Зроблено (Сесія 14):**
- `src/components/EmptyState.tsx` — навмисно мінімальний (текст по центру, без іконок/ілюстрацій — MVP-масштаб, легко розширити пізніше)
- `page.tsx` — додано `count`-запит (`.select("*", { count: "exact", head: true })`, тягне лише лічильник, не самі рядки) для `photos`, передає `hasAnyPhotos` у `Gallery`
- `Gallery.tsx` — якщо `!hasAnyPhotos`, взагалі не рендерить `GalleryTabs`/`GalleryGrid`, тільки `EmptyState` ("Фото ще немає — портфоліо скоро з'явиться тут"). Це рівень "усього сайту", окремий від рівня таба
- `GalleryGrid` (той самий файл) — якщо після завершення першого завантаження (`!isLoading`) `photos.length === 0`, показує `EmptyState` ("У цьому табі поки немає фото") замість порожньої сітки колонок — це рівень окремого таба, спрацьовує коли інші таби непорожні, а обраний — так

**Перевірка (обидві сесії):** `npm run build`/`lint`/`test` чисті (21/21 — нової чистої логіки немає, `PUT /api/admin/content` і умовний рендер `Gallery`/`GalleryGrid` це wiring, не бізнес-логіка з "Підходу до тестування"). `next dev` + `curl`:
- `/admin/content` без сесії → `307`; `PUT /api/admin/content` без сесії → `401` (і з валідним, і з вигаданим `key` — авторизаційна перевірка йде першою, тому білий список ключів окремо ізольовано не протестовано через curl, лише читанням коду)
- Публічна `/` з реальними 24 фото — `<html lang="uk">`, у розмітці 0 входжень "Фото ще немає" (порожній стан не зачепив живі дані)
- Тимчасовий рендер-тест (`Gallery.tmp.test.tsx`, `renderToStaticMarkup`, видалений одразу після прогону, у git не потрапив): `hasAnyPhotos={false}` → в розмітці є "Фото ще немає", немає назви жодного таба (тобто `GalleryTabs` дійсно не рендериться)
- Реальне видалення фото з живої БД користувача для перевірки порожніх станів **не робив** — той самий принцип, що і скрізь: не чіпати живий контент без прямого дозволу; логіка перевірена ізольовано рендер-тестом замість цього

**Не зроблено / далі:**
- Ручна перевірка "зміна імені в адмінці → видно на публічній `/`" — вимагає реальної сесії, відкладено до після деплою за проханням користувача
- `EmptyState` не використовується більше ніде, крім `Gallery`/`GalleryGrid` — план і не просив, просто фіксую для майбутніх сесій, якщо знадобиться той самий компонент десь ще

**Нотатки для наступної сесії:**
- Наступний крок — Сесія 15 (деплой), підготовча частина коду вже зроблена (див. запис нижче), лишається сам деплой руками користувача

### Session 15 (частково) — 2026-07-11

**Зроблено:**
- `<Analytics />` з `@vercel/analytics/react` підключено в `src/app/layout.tsx` (пакет був у залежностях зі старту проєкту, Сесія 0 — просто не використовувався)
- Попутний фікс: `<html lang="en">` → `lang="uk"` — це лишалось з дефолтного `create-next-app`-скаффолду й було просто пропущено раніше; весь контент сайту українською, `lang="en"` вводив в оману скрін-рідери й пошукові системи. Не було в жодному чекбоксі плану, зафіксовано тут як знахідку під час роботи
- `vercel.json` не створювався — план позначає його як "за потреби", жодних кастомних редіректів/заголовків не виникло, стандартний Vercel-автодетект Next.js обходиться без нього
- `npm run build`/`lint`/`test` чисті (21/21) на момент запису

**Не зроблено — і не може бути зроблено асистентом:**
- Сам деплой (Import Project на vercel.com, env-змінні в Vercel Dashboard, публічний URL) вимагає акаунту Vercel користувача — асистент не має до нього доступу і не виконує OAuth/зовнішні логіни самостійно
- Усі ручні перевірки з мобільного (Instagram in-app browser, повний цикл адмінки з телефону) — за проханням користувача зведені в один прохід після деплою, а не розкидані по кожній сесії окремо

**Нотатки для користувача (не для наступної сесії — далі сесій за планом не лишилось):**
1. vercel.com → Import Project → підключити `Feodosij/insta-portfolio`
2. Settings → Environment Variables — перенести всі змінні з `.env.example`/локального `.env`
3. Деплой → відкрити лінк з телефону через Instagram in-app browser
4. Пройти одним заходом усі відкладені ручні перевірки з Сесій 9-14 (перелічені в чекбоксах цих сесій вище) + перевірки, специфічні для Сесії 15 (hero/галерея/viewer/послуги/OG-прев'ю, повний цикл адмінки з мобільного)
5. Якщо щось не зійдеться — це буде окрема сесія на фікс, план MVP на цьому вичерпано (Сесія 15 — остання за `docs/development-plan.md`)
